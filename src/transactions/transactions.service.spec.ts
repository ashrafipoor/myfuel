...
import { ApprovedTransactionResponse } from './dto/transaction-response.dto';
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(LimitCounter),
          useValue: createMockRepository(),
        },
        {
          provide: getRepositoryToken(OrgBalance),
          useValue: createMockRepository(),
        },
        {
  ...
  describe('processTransaction', () => {
    it('should process a valid transaction successfully', async () => {
      ...
      mockDataSource.transaction.mockImplementation(
        (callback: (manager: MockRepository) => Promise<never>) => {
          const manager = {
            findOne: jest
              .fn()
              .mockResolvedValueOnce(mockBalance) // for OrgBalance
              .mockResolvedValue(null), // for LimitCounters
            create: jest.fn((_: unknown, obj: object) => obj),
            save: jest.fn((obj: object) =>
              Promise.resolve({ ...obj, id: 'new-uuid' }),
            ),
      ...
      const result = (await service.processTransaction(
        transactionDto,
        'idempotency-key',
      )) as ApprovedTransactionResponse;
